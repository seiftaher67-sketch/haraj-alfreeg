...
const ProfileHome = () => {
  ...
  const [showMenu, setShowMenu] = useState(true); // 👈 للتحكم في إظهار القوائم على الموبايل
  const { unreadCount } = useNotifications();

  const subMenuItems = [
    { path: "profile-edit", label: "تعديل البروفايل", icon: <User className="w-4 h-4" /> },
    { path: "location", label: "الموقع", icon: <MapPin className="w-4 h-4" /> },
    { path: "change-password", label: "تغيير كلمة المرور", icon: <Lock className="w-4 h-4" /> },
  ];
  ...
  const menuItems = [
    { path: "account-payments", label: "الحساب والدفع", icon: <CreditCard className="w-5 h-5" /> },
    { path: "recharge", label: "شحن الحساب", icon: <Wallet className="w-5 h-5" /> },
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      {/* محتوى الصفحة */}
      <div className="flex-1 ml-0 lg:ml-14 overflow-y-auto p-4">
